...
class UsuarioControllerTest {
    ...
    void deveCriarUsuarioComSucesso() throws Exception {
        // Arrange
        UsuarioRequestDto request = new UsuarioRequestDto("Arthur", "12345678901", "arthur@email.com", "senha123");
    ...
    void deveRetornar404QuandoUsuarioNaoExiste() throws Exception {
        ...
        Long id = 99L;
        String mensagemErro = "Usuário com id " + id + " não encontrado";


        when(service.buscarUsuarioId(id)).thenThrow(new UsuarioNotFoundException(id));


        mockMvc.perform(get("/usuarios/" + id))
                .andExpect(status().isNotFound()) // Verifica se o status é 404
                .andExpect(jsonPath("$.erro").value(mensagemErro)) // Verifica se a mensagem no JSON bate
    ...
    void deveAtualizarUsuarioComSucesso() throws Exception {
        Long id = 1L;
        UsuarioRequestDto request = new UsuarioRequestDto("Arthur Silva", "12345678901", "arthur.novo@email.com", "senha123");
    ...
    @Test
    @DisplayName("Deve deletar um usuário e retornar 204 No Content")
